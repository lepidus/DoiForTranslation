...
class SubmissionsTranslationPlugin extends GenericPlugin
{
    ...
    public function addWorkflowModifications($hookName, $params)
    {
        ...
        if($templateMgr->getTemplateVars('requestedPage') == 'workflow' and $submissionIsNotTranslation) {
            ...
            $translationsForDisplay = $translationsService->getTranslationsForDisplay($submission->getId(), 'workflow');
            $templateMgr->assign([
    ...
    public function addArticlePageModifications($hookName, $params)
    {
        ...
        if($submissionIsNotTranslation) {
            $translationsService = new TranslationsService();
            $translations = $translationsService->getTranslationsForDisplay($submission->getId(), 'articlePage');

            if(count($translationsForDisplay) > 0) {
                $templateMgr->assign([
                    'translations' => $translationsForDisplay
                ]);
